...
const negotiable = ref(false),
  ...
  ageUnit = ref("months");
const computedAgeValue = computed<number | undefined>(() =>
  age.value === null ? undefined : age.value
);
const gender = ref(""),
...
const feedingType = ref(""),
  detailedLocation = ref(""),
  deliveryOptions = ref<string[]>([]);
const deliveryRestrictions = ref(""),
  maxDeliveryDistance = ref<number | null>(null),
  deliveryDistanceUnit = ref("miles");
const certified = ref(false),
  auction = ref(false),
  availableImmediate = ref(false);
const imagePreviewUrls = ref<string[]>([]),
  videoPreview = ref<string | undefined>(undefined),
...
const goToStep = (step: number) => {
  if (
    ...
    stepValidation.value[step as keyof typeof stepValidation.value]
  ) {
...
const handleFileUpload = async (event: Event) => {
  ...
  const fileArray = Array.from(files).slice(0, maxImageUploadCount - imagePreviewUrls.value.length);

  const uploadPromises = fileArray.map(async (file, index) => {
    const uniqueFilePath = `images/${Date.now()}_${index}_${file.name}`;

    try {
      const publicUrl = await upload(file, uniqueFilePath);
      if (publicUrl) {
        imagePreviewUrls.value.push(publicUrl);
      }
    } catch (err) {
...
const submitForm = () => {
  const formData: AnimalListing = {
    ...
    age: age.value,
    age_unit: ageUnit.value,
    gender: gender.value,
    weight: weight.value,
    weight_unit: weightUnit.value,
    health_status: healthStatus.value,
    feeding_type: feedingType.value,
    location: detailedLocation.value,
    delivery_option: deliveryOptions.value,
    delivery_restrictions: deliveryRestrictions.value,
    max_delivery_distance: maxDeliveryDistance.value,
    delivery_distance_unit: deliveryDistanceUnit.value,
...
<template>
  <div
    class="min-h-screen bg-gradient-to-br from-emerald-50 to-green-50 font-sans antialiased text-gray-800"
  >
    <!-- Toast Notification -->
    <div
      v-if="toastVisible"
    ...
    >
      <div class="flex items-center">
        <div
          class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-white bg-opacity-25"
        >
          <CheckCircleIcon
    ...
    <header
      class="bg-white shadow-md lg:hidden sticky top-0 z-40 border-b border-green-100"
    >
      ...
      <div class="px-4 py-3 bg-emerald-50">
        ...
        <div
          class="h-2.5 bg-gray-100 rounded-full overflow-hidden shadow-inner"
        >
          <div
      ...
      >
        <div class="p-5 border-b border-gray-100">
          ...
          <p class="text-sm text-gray-600">
            Complete your listing in 5 easy steps
          </p>
        </div>
        ...
        <nav class="p-5">
          <ul class="space-y-3">
            <li v-for="(step, index) in stepsItems" :key="index">
              ...
              >
                <div
                  class="flex items-center justify-center h-8 w-8 rounded-full mr-3"
                  :class="[
                    currentStep === index + 1
                ...
                <CheckCircleIcon
                  ...
                  class="ml-auto w-5 h-5 text-emerald-500"
                />
                <span
                  v-else-if="index + 1 === currentStep"
                  class="ml-auto w-2 h-2 rounded-full bg-white animate-pulse"
                ></span>
              </button>
            </li>
          </ul>
        ...
        <div class="p-5 mt-4">
          <div
            class="p-4 bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl border border-emerald-100 shadow-sm"
          >
            ...
            <p class="text-sm text-emerald-600">
              Add multiple high-quality photos to increase interest in your
              listing by up to 250%!
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="flex flex-col lg:flex-row">
      ...
      >
        ...
        <div
          class="bg-gradient-to-br from-emerald-600 to-green-700 text-white px-6 py-8 bg-opacity-100"
        >
          ...
          <button
            @click="goBack"
            class="flex items-center gap-2 bg-white bg-opacity-15 hover:bg-opacity-25 text-white px-4 py-2.5 rounded-lg transition-all duration-300 w-full text-sm font-medium shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-30"
          >
            <ArrowLeftIcon class="w-5 h-5" />
        ...
        <div class="p-5 border-b border-gray-100">
          ...
          <div
            class="h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner"
          >
            <div
              class="h-full bg-gradient-to-r from-emerald-400 to-green-500 transition-all duration-500 ease-out rounded-full"
              :style="`width: ${progressPercentage}%`"
          ...
          <div
            class="mt-2 text-xs font-medium text-gray-600 flex justify-between"
          >
            <span
              >Step {{ currentStep }} of {{ totalSteps }}:
              {{ stepsItems[currentStep - 1].title }}</span
        ...
        <nav class="p-5 overflow-y-auto">
          <h3
            class="text-xs font-bold uppercase text-gray-500 mb-3 tracking-wider px-2"
          >
            Form Steps
          </h3>
          <ul class="space-y-2">
            <li v-for="(step, index) in stepsItems" :key="index">
              ...
              >
                <div
                  class="flex items-center justify-center h-9 w-9 rounded-full mr-3"
                  :class="[
                    currentStep === index + 1
        ...
        <div class="max-w-4xl mx-auto">
          ...
          <div class="hidden lg:block mb-6">
            ...
            <p class="text-gray-500 ml-12">
              Complete all steps to create your premium livestock listing
            </p>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-md overflow-hidden mb-8">
            ...
            <LivestockFormReview
              ...
              :delivery-restrictions="deliveryRestrictions"
              :max-delivery-distance="maxDeliveryDistance ?? undefined"
              :delivery-distance-unit="deliveryDistanceUnit"
              :certified="certified"
              :auction="auction"
              :available-immediate="availableImmediate"
              :image-preview-urls="imagePreviewUrls"
              :video-preview="videoPreview"
              :category-options="categoryOptions"
              :available-breeds="availableBreeds"
              @editForm="editForm"
            ...
            <LivestockFormSteps
              ...
              :available-immediate="availableImmediate"
              :image-preview-urls="imagePreviewUrls"
              :video-preview="videoPreview ?? undefined"
              :current-step-valid="currentStepValid"
              :available-breeds="availableBreeds"
              @update:title="title = $event"
              @update:description="description = $event"
              @update:category="category = $event"
              @update:breed="breed = $event"
              @update:price="price = $event"
              @update:negotiable="negotiable = $event"
              @update:quantity="quantity = $event"
              @update:age="age = $event"
              @update:ageUnit="ageUnit = $event"
              @update:gender="gender = $event"
              @update:weight="weight = $event"
              @update:weightUnit="weightUnit = $event"
              @update:healthStatus="healthStatus = $event"
              @update:feedingType="feedingType = $event"
              @update:detailedLocation="detailedLocation = $event"
              @update:deliveryOptions="deliveryOptions = $event"
              @update:deliveryRestrictions="deliveryRestrictions = $event"
              @update:maxDeliveryDistance="maxDeliveryDistance = $event"
              @update:deliveryDistanceUnit="deliveryDistanceUnit = $event"
              @update:certified="certified = $event"
              @update:auction="auction = $event"
              @update:availableImmediate="availableImmediate = $event"
              @nextStep="nextStep"
              @prevStep="prevStep"
          ...
          <!-- Footer -->
          <div
            class="mt-8 text-center text-sm text-gray-500 border-t border-gray-100 pt-6"
          >
            <p class="flex items-center justify-center gap-3 flex-wrap">
              <span>Livestock Market © 2025</span>
              <span
                class="w-1.5 h-1.5 rounded-full bg-gray-300 hidden sm:block"
              ></span>
              <a
                href="#"
                class="text-emerald-600 hover:text-emerald-700 hover:underline transition-colors"
                >Terms of Service</a
              >
              <span
                class="w-1.5 h-1.5 rounded-full bg-gray-300 hidden sm:block"
