...
export default defineComponent({
    ...
    components: {
        ...
        Badge,
        Paginator,
        ViewListings // Register the component
    },
    setup() {
        ...
        // State for viewing a specific listing
        const selectedListingId = ref<number | null>(null);
        const showListingDetails = ref(false);
        ...
        // Method to get the selected listing details
        const getSelectedListing = computed(() => {
            return listings.value.find(listing => listing.id === selectedListingId.value) || null;
        });

        // Updated method to open the listing details
        const viewListing = (id: number) => {
            selectedListingId.value = id;
            showListingDetails.value = true;
            console.log(`Opening listing ${id} details`);
        };

        // Method to close the listing details view
        const closeListingDetails = () => {
            showListingDetails.value = false;
...
<template>
    <div class="livestock-marketplace">
        <!-- ViewListings component (conditional rendering) -->
        <ViewListings v-if="showListingDetails" :listing="getSelectedListing" :id="getSelectedListing?.id ?? 0"
            @close="closeListingDetails" />

        <!-- Hide the marketplace when viewing details -->
        <div v-if="!showListingDetails">
            ...
            <div class="container mx-auto px-4 py-8">
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <div class="flex items-center justify-between mt-auto">
                                <div class="flex items-center">
                                    <button
                                        class="w-8 h-8 flex items-center justify-center rounded-full mr-2 transition-all duration-200"
                                        :class="[listing.isFavorite ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-500']"
                                        @click="toggleFavorite(listing)" aria-label="Toggle favorite">
                                        <i class="pi pi-heart-fill text-sm"></i>
                                    </button>
                                    <span class="text-sm text-gray-500">{{ listing.quantity }} available</span>
                                </div>
                                <Button label="View Details" icon="pi pi-arrow-right" iconPos="right"
                                    class="p-button-rounded p-button-success text-sm"
                                    @click="viewListing(listing.id)" />
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="min-w-full bg-white rounded-lg overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="listing in filteredListings" :key="listing.id"
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 flex-shrink-0 rounded-md overflow-hidden mr-3">
                                            <img :src="listing.imageUrl" :alt="listing.title"
                ...
                <!-- Empty state -->
                <div v-if="filteredListings.length === 0" class="p-12 text-center">
                    <div class="mx-auto w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                        <i class="pi pi-inbox text-gray-400 text-4xl"></i>
                    </div>
                    <h3 class="text-lg font-medium text-gray-700 mb-2">No listings found</h3>
                    <p class="text-gray-500 mb-6">Try changing your search criteria or check back later.</p>
                ...
                <div class="pt-6 flex justify-center">
                    <Paginator v-model:first="first" :rows="9" :totalRecords="filteredListings.length"
